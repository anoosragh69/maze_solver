...
\section*{Abstract}
This report presents a maze-solving system built with a Genetic Algorithm (GA). The maze is generated using \texttt{pyamaze}, and candidate movement sequences are evolved over generations to produce valid paths from the start cell \((rows, cols)\) to the goal cell \((1,1)\). In addition to path feasibility, the project evaluates solution quality under two objectives: travel time and traversal cost. The final system extracts and visualizes three representative solutions from a single evolved population: fastest, cheapest, and balanced.
...
\begin{itemize}
    ...
    \item Objective optimization is post-processing based; a true multi-objective GA (e.g., Pareto ranking) could improve trade-off quality.
